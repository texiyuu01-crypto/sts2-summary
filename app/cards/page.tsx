...
const parseDescription = (card: SpireCard) => {
  ...
  if (card.keywords && card.keywords.length > 0) {
    ...
    if (topKeywords.length > 0) {
      const topHtml = topKeywords
        .map(kw => `<span style="color: #82eefd; font-weight: 800;">${kw}。</span>`)
        .join(' ');
    ...
    if (exhaustKeywords.length > 0) {
      const bottomHtml = exhaustKeywords
        .map(kw => `<span style="color: #82eefd; font-weight: 800;">${kw}。</span>`)
        .join(' ');
...
const CardFrameStroke = ({ type, color }: { type: string, color: string }) => {
  ...
  let path = "";
  if (t.includes('attack') || t.includes('アタック')) path = "M0,0 H100 V100 L50,130 L0,100 Z";
  else if (t.includes('power') || t.includes('パワー')) path = "M0,0 H100 V100 C100,135 0,135 0,100 Z";
  else path = "M0,0 H100 V125 H0 Z";
...
const SortableCard = ({ card, isOverlay = false, onHover }: { 
  card: SpireCard, 
  isOverlay?: boolean, 
  onHover?: (card: SpireCard | null, e?: React.MouseEvent) => void 
}) => {
  ...
  return (
    <div 
      ...
      className={`relative w-14 h-[85px] md:w-16 md:h-[95px] flex flex-col group cursor-grab active:cursor-grabbing ${isOverlay ? 'scale-110 z-[300]' : ''}`}
    >
      <div className="relative w-full h-full overflow-hidden pointer-events-none">
        ...
        <div className="flex-1 flex items-start justify-center px-0.5 pt-1 z-20 overflow-visible">
          <p className="card-name-text font-bold text-[#ffffff] text-center uppercase break-words w-full"
             style={{ fontSize: '7.5px', lineHeight: '1.1', display: 'block', minHeight: '2.2em' }}>
            {card.name}
          </p>
        </div>
...
const TierRow = ({ tier, cards, onHover }: { 
  tier: typeof TIER_ROWS[0], 
  cards: SpireCard[],
  onHover: (card: SpireCard | null, e?: React.MouseEvent) => void
}) => {
  ...
  return (
    <div className="flex border-b border-[#1e293b] min-h-[105px] bg-[#0d0d12]">
      <div 
        ...
        className="w-12 md:w-20 flex items-center justify-center shrink-0 border-r border-[#000000] z-20"
      >
      ...
      <div ref={setNodeRef} className="flex-1 p-3 flex flex-wrap gap-x-2 gap-y-6 content-start min-w-[200px]">
        <SortableContext items={cards.map(c => c.id)} strategy={horizontalListSortingStrategy}>
          {cards.map(card => <SortableCard key={card.id} card={card} onHover={onHover} />)}
        </SortableContext>
      </div>
    </div>
  );
...
export default function CardsPage() {
  ...
  useEffect(() => {
    fetch('https://spire-codex.com/api/characters?lang=jpn')
      .then(res => res.json())
      .then(data => {
        const sortedChars = (data as {id: string, name: string}[]).sort((a, b) => {
  ...
  useEffect(() => {
    ...
    fetch(`https://spire-codex.com/api/cards?lang=jpn${colorQuery}`)
      .then(res => res.json())
      .then(data => {
        setAllCards(data);
  ...
  const handleHover = (card: SpireCard | null, e?: React.MouseEvent) => {
    if (activeId) return;
    setHoveredCard(card);
    if (e) setMousePos({ x: e.clientX + 15, y: e.clientY - 120 });
  };
  ...
  const handleDragOver = (event: any) => {
    ...
    setTierData(prev => {
      ...
      return {
        ...
        [overContainer]: [...prev[overContainer].slice(0, newIndex), prev[activeContainer][activeIndex], ...prev[overContainer].slice(newIndex)]
      };
    });
  };
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (activeContainer && overContainer && activeContainer === overContainer) {
      ...
      if (activeIndex !== overIndex) {
        setTierData(prev => ({ ...prev, [overContainer]: arrayMove(prev[overContainer], activeIndex, overIndex) }));
      }
    }
    setActiveId(null);
  };
    ...
    setTimeout(async () => {
      try {
        const canvas = await html2canvas(tierRef.current!, {
          ...
          ignoreElements: (el) => el.tagName === 'NAV' || el.classList.contains('dnd-kit-drag-overlay')
        });
  ...
  return (
    <main className="min-h-screen bg-[#0d0d12] text-[#e2e8f0] p-4 md:p-8 font-sans">
      <nav className="max-w-7xl mx-auto mb-6 flex justify-between items-center">
        <Link href="/" className="text-[10px] font-black text-[#64748b] hover:text-[#60a5fa] transition-colors uppercase tracking-[0.2em]">← Return to Nexus</Link>
        <button onClick={() => { setIsTierMode(!isTierMode); setHoveredCard(null); }} className={`px-4 py-1.5 text-[9px] font-black rounded-sm border transition-all ${isTierMode ? 'bg-[#ffffff] text-[#000000] border-[#ffffff]' : 'bg-[#1d4ed81a] text-[#60a5fa] border-[#3b82f6] hover:bg-[#2563eb] hover:text-[#ffffff]'}`}>
          {isTierMode ? 'EXIT EDITOR' : 'TIER MAKER'}
        </button>
      </nav>

      <div className="max-w-7xl mx-auto mb-10 overflow-x-auto pb-4 scrollbar-hide text-center">
        <div className="inline-flex gap-1.5 p-1 bg-[#0f172a] rounded-sm border border-[#ffffff1a]">
          ...
          {characters.map((char) => (
            <button key={char.id} onClick={() => setActiveTab(char.id)} className={`px-4 py-1.5 rounded-sm text-[9px] font-black uppercase transition-all ${activeTab === char.id ? 'bg-[#2563eb] text-[#ffffff]' : 'text-[#64748b] hover:text-[#cbd5e1]'}`}>{char.name}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="max-w-5xl mx-auto">
          <DndContext sensors={sensors} collisionDetection={rectIntersection} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
            <div className="flex justify-end mb-4">
              <button onClick={exportPNG} className="text-[9px] font-black text-[#60a5fa] hover:text-[#ffffff] border border-[#3b82f64d] px-3 py-1 rounded-sm uppercase">Export PNG</button>
            </div>
            
            <div ref={tierRef} className="border border-[#1e293b] rounded-sm overflow-hidden mb-8 bg-[#020617] shadow-2xl">
              {TIER_ROWS.map(tier => <TierRow key={tier.id} tier={tier} cards={tierData[tier.id]} onHover={handleHover} />)}
            </div>
            ...
            <div className="bg-[#0f172a80] p-6 border border-[#ffffff0d] rounded-sm">
              ...
              <div className="flex flex-wrap gap-x-2 gap-y-8 min-h-[150px]">
                <SortableContext items={tierData.pool.map(c => c.id)} strategy={horizontalListSortingStrategy}>
                  {tierData.pool.map(card => <SortableCard key={card.id} card={card} onHover={handleHover} />)}
                </SortableContext>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-x-2 gap-y-12">
          {allCards.map((card) => {
            const color = getRarityColor(card.rarity);
            return (
              <div key={card.id} className="group relative flex flex-col transition-all hover:scale-110 hover:z-50 cursor-crosshair"
                onMouseEnter={(e) => handleHover(card, e)} onMouseLeave={() => setHoveredCard(null)}
                onMouseMove={(e) => setMousePos({ x: e.clientX + 15, y: e.clientY - 120 })}
              >
                <div className="relative aspect-[1/1.32] w-full flex flex-col overflow-hidden">
                  <div className="w-full aspect-square relative z-0 bg-[#0f172a] border-t border-x border-[#ffffff0d]">
                    <img src={formatImageUrl(card.image_url)} alt="" className="w-full h-full object-contain" />
                    <div className="absolute top-1 left-1 z-30">
                      <div className="w-4 h-4 bg-[#000000cc] border border-[#ffffff33] rounded-full flex items-center justify-center">
                        <span className="text-[8px] font-black italic" style={{ color }}>{card.cost === -1 ? 'X' : card.cost}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex-1 flex items-start justify-center px-1 pt-1.5 z-20">
                    <p className="text-[7px] font-black text-[#ffffff] text-center leading-tight uppercase tracking-tighter">{card.name}</p>
                  </div>
                  <CardFrameStroke type={card.type} color={color} />
                </div>
              </div>
