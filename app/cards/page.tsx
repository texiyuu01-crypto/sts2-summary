    ...
    <main className="min-h-screen bg-slate-900 text-slate-200 p-8">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {allCards.map((card) => (
          <div 
            key={card.id} 
            className="bg-slate-800 border border-slate-700 rounded-xl overflow-hidden hover:border-yellow-500 transition-all group shadow-lg"
          >
            ...
            <div className="p-4 space-y-3">
              ...
              <p 
                ...
                dangerouslySetInnerHTML={{ 
                  __html: (card.text || "効果テキストなし").replace(/\[|\]/g, '') 
                }}
              />
            </div>
          </div>
